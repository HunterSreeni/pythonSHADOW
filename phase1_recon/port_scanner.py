#!/usr/bin/env python3
"""
Port scanning module using native Python and nmap wrapper.

Usage:
    python port_scanner.py --target example.com --output results/
"""
...
from core.utils import setup_logging, load_config, timestamp_now, ensure_dir

logger = setup_logging("port_scanner")
...
class PortScanner:
    """
    Port scanner with native Python and nmap support.

    Features:
    - Native async TCP connect scanning
    - Nmap wrapper for advanced scanning
    - Service detection
    - Banner grabbing
    """
    ...
    def _parse_ports(self, ports: str) -> List[int]:
        ...
        if ports == "top-1000":
            ...
            return TOP_1000_PORTS[:100]
        elif ports == "common":
            return list(COMMON_PORTS.keys())
        elif ports == "all" or ports == "1-65535":
    ...
    async def scan(self) -> ScanResult:
        ...
        logger.info(f"Starting port scan on {len(self.targets)} targets, {len(self.ports_to_scan)} ports")

        for target in self.targets:
            try:
                # Resolve hostname
                ip = await self._resolve_host(target)
                if not ip:
                    result.add_error(f"Could not resolve: {target}")
                    continue

                host_result = HostResult(host=target, ip=ip)

                if self.use_nmap and self._nmap_available():
                    # Use nmap for scanning
                    host_result = await self._nmap_scan(target, host_result)
                else:
                    # Native Python scanning
                    host_result = await self._native_scan(ip, host_result)
                ...
                for port in host_result.ports:
                    if port.state == "open":
                        severity = self._get_port_severity(port.port, port.service)
                        result.add_finding(Finding(
                            title=f"Open Port: {port.port}/{port.service or 'unknown'}",
                            severity=severity,
                            description=f"Open port detected on {target}",
                            url=f"{target}:{port.port}",
                            metadata={
                                "ip": ip,
                                "service": port.service,
                                "version": port.version,
                                "banner": port.banner[:200] if port.banner else "",
                            },
                        ))

            except Exception as e:
                result.add_error(f"Error scanning {target}: {e}")
    ...
    async def _resolve_host(self, host: str) -> Optional[str]:
        ...
        try:
            ...
            info = await loop.getaddrinfo(host, None, socket.AF_INET)
            if info:
                return info[0][4][0]
        except Exception as e:
    ...
    async def _native_scan(self, ip: str, host_result: HostResult) -> HostResult:
        """Native Python TCP connect scan."""
        logger.info(f"Scanning {ip} ({len(self.ports_to_scan)} ports)...")

        semaphore = asyncio.Semaphore(self.threads)

        async def scan_port(port: int) -> Optional[Port]:
            async with semaphore:
                try:
                    ...
                    if self.verbose:
                        return Port(port=port, state="filtered")
                except ConnectionRefusedError:
                    if self.verbose:
                        return Port(port=port, state="closed")
                except Exception:
                    pass
                return None

        tasks = [scan_port(port) for port in self.ports_to_scan]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for result in results:
            if isinstance(result, Port) and result.state == "open":
                host_result.ports.append(result)

        logger.info(f"Found {len(host_result.ports)} open ports on {ip}")
        return host_result
    ...
    async def _nmap_scan(self, target: str, host_result: HostResult) -> HostResult:
        """Use nmap for scanning."""
        logger.info(f"Running nmap on {target}...")

        port_spec = ",".join(str(p) for p in self.ports_to_scan[:1000])  # Limit for nmap

        try:
            proc = await asyncio.create_subprocess_exec(
                "nmap", target,
                "-p", port_spec,
                *self.nmap_args.split(),
                "-oX", "-",
            ...
            # Parse XML output
            host_result = self._parse_nmap_xml(stdout.decode(), host_result)
    ...
    def _parse_nmap_xml(self, xml_output: str, host_result: HostResult) -> HostResult:
        """Parse nmap XML output."""
        import re

        # Extract ports
        port_pattern = r'<port protocol="tcp" portid="(\d+)".*?<state state="(\w+)".*?(?:<service name="([^"]*)".*?(?:version="([^"]*)")?)?'

        for match in re.finditer(port_pattern, xml_output, re.DOTALL):
            port_num = int(match.group(1))
            state = match.group(2)
            service = match.group(3) or COMMON_PORTS.get(port_num, "")
    ...
    def _get_port_severity(self, port: int, service: str) -> Severity:
        ...
        # High-risk services
        high_risk = ["telnet", "ftp", "rsh", "rlogin", "vnc", "rdp", "smb"]
        if service.lower() in high_risk or port == 23:
    ...
    def save_results(self, result: ScanResult) -> Dict[str, str]:
        ...
        target_name = self.targets[0] if len(self.targets) == 1 else "multi"
        paths = self.result_manager.save(result, f"ports_{target_name}")

        # Save detailed JSON
        detailed_path = self.output_dir / f"ports_{target_name}_detailed.json"
        with open(detailed_path, "w") as f:
            json.dump(
                {
                    "timestamp": timestamp_now(),
                    "targets": self.targets,
                    "results": {h: r.to_dict() for h, r in self.results.items()},
                },
                f,
                indent=2,
            )
...
async def main():
    ...
    parser.add_argument("-t", "--target", required=True, help="Target host(s), comma-separated")
    parser.add_argument("-o", "--output", default="results", help="Output directory")
    parser.add_argument("-p", "--ports", default="top-1000", help="Ports to scan")
    ...
    parser.add_argument("--nmap", action="store_true", help="Use nmap")
    parser.add_argument("--nmap-args", default="-sV", help="Nmap arguments")
    ...
    paths = scanner.save_results(result)

    print(f"\n{'='*60}")
    print(f"Port Scan Complete")
    print(f"{'='*60}")
    print(f"Hosts Scanned: {result.stats.get('hosts_scanned', 0)}")
    print(f"Open Ports: {result.stats.get('open_ports', 0)}")
    print(f"\nResults saved to:")
